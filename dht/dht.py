...
findqueue = multiprocessing.Queue()
findlock = Lock()
findlist = []
...
Returns either None (if no response was found) or [entries received, sender].
"""
def get_reply_find_node(e, nonce):
    t0 = time.time()
    while time.time() < t0+timeout:
        try:
            (message, a) = findqueue.get(True, 1)
            # remember the first message encountered
            orig = (message, a)
            m = struct.unpqck_from('>BIHIH', message)
            mtype = m[0]
            msender = m[2]
            mnonce = m[4]
            if mtype == 5 and mnonce == nonce:
                elen = struct.unpack_from('>I', message, 13)[0]
                entries = []
                for i in range(0, elen, entrysize):
                    e = struct.unpack_from(entryformat, message, 17+i)
                    entry = (socket.inet_ntoa(struct.pack('>I', e[1])), e[2], e[4])
                    entries.append(entry)
                return [entries, msender]
            else:
                # put message back if not the correct message
                findqueue.put((m, a))
        except:
            continue
        # if message was found, and did not match our nonce, check the rest of the queue
        while True:
            try:
                (message, a) = findqueue.get(True, 1)
                # if looped through all messages, try again
                if (message, a) == orig:
                    break
        ...
        if s[0]:
            ...
            try:
                ...
                reply_error(address, 0, 'improper message format')
            if mtype == 5 and mnonce == nonce:
                elen = struct.unpack_from('>I', reply, 13)[0]
                entries = []
                for i in range(0,elen,entrysize):
                    en = struct.unpack_from(entryformat, reply, 17 + i)
                    entry = (socket.inet_ntoa(struct.pack('>I',en[1])), en[2], en[4])
                    entries.append(entry)
                return [entries, msender]
            else:
                continue
...
def findkclosest(key):
    ...
    while True:
        ...
        for e in closest[:K]:
            if e not in pinged:
                nonce = send_find_node((e[0],e[1]),e[2])
                nextping.append((e,nonce))
                pinged.append(e)
                # check if new nodes found
                if len(nextping) == a:
                    break
        responses = [pool.apply_async(get_reply_find_node, (e,n)) for e,n in nextping]
        for res in responses:
            # got a response
            if isinstance(res,list):
                if res[1] not in responded:
        ...
        if all([e[2] in responded for e in closest[:K]]):
            return pinged[:K]
